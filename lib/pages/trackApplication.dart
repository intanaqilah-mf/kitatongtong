...
class TrackApplicationsScreen extends StatefulWidget {
  @override
  _TrackApplicationsScreenState createState() => _TrackApplicationsScreenState();
}
...
class _TrackApplicationsScreenState extends State<TrackApplicationsScreen>
  ...
  Stream<QuerySnapshot> _getApplicationsStream(int tabIndex) {
    // This query now fetches all applications from the collection.
    Query query = FirebaseFirestore.instance.collection('applications');

    // This part remains the same to handle filtering by tabs
    switch (tabIndex) {
      case 1: // Submitted
    ...
    // This part remains the same to handle sorting
  ...
  List<DocumentSnapshot> _filterDocsBySearch(List<DocumentSnapshot> docs) {
    ...
    return docs.where((doc) {
      ...
      return code.contains(_searchQuery) || fullname.contains(_searchQuery);
    }).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text(
          localizations.translate('track_applications_title'),
  ...
  Widget _buildApplicationCard(DocumentSnapshot doc) {
    ...
    final date = dateStr.isNotEmpty
        ? DateFormat("dd MMM yy").format(DateTime.parse(dateStr))
    ...
    final submittedByData = data['submittedBy'];
    String submittedByName;
    if (submittedByData is Map) {
      submittedByName = submittedByData['name'] ?? 'N/A';
    } else if (submittedByData is String) {
      submittedByName = submittedByData;
    } else {
      submittedByName = 'N/A';
    }
